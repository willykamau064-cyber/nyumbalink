...
app.get("/listings", (req, res) => res.sendFile(path.join(__dirname, "templates", "listings.html")));
app.get("/agents", (req, res) => res.sendFile(path.join(__dirname, "templates", "agents.html")));
app.get("/neighborhoods", (req, res) => res.sendFile(path.join(__dirname, "templates", "neighborhoods.html")));
app.get("/services", (req, res) => res.sendFile(path.join(__dirname, "templates", "services.html")));
// Legacy redirects
...
app.post("/api/rate", async (req, res) => {
  const { rating, user_email, comment } = req.body;
  const { data, error } = await supabase.from("ratings").insert([
    { rating: parseInt(rating), user_email, comment }
  ]);
  if (error) return res.status(400).json({ success: false, message: error.message });
  res.json({ success: true, message: "Rating submitted", data });
});
