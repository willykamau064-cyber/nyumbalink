...
async function test() {
    try {
        ...
        const token = JSON.parse(d1).access_token;
        ...
        const r2 = await fetch('https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest', {
            ...
            body: JSON.stringify({
                BusinessShortCode: SHORTCODE, Password: pwd, Timestamp: ts, TransactionType: 'CustomerPayBillOnline',
                Amount: 1, PartyA: p, PartyB: SHORTCODE, PhoneNumber: p, CallBackURL: 'https://sandbox.safaricom.co.ke/',
                AccountReference: 'LinkPointHub', TransactionDesc: 'Test Payment'
            })
        });
