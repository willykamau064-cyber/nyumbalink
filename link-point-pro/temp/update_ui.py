...
for f in html_files:
    ...
    if 'chat-btn' not in content:
        # Add 'Amenities' button directly beneath the features of each property card
        content = re.sub(
            r'<div class="pfeats">((?:.|\n)*?)</div>\s*</div>\s*</div>',
            r'<div class="pfeats">\1</div><div class="amenity-btn" onclick="openAmenities(\'Neighbourhood\')"><i class="fas fa-map-signs"></i> View Amenities</div></div></div>',
            content
        )
        
        # Add dedicated Feedback / Ratings button into Navbar for visibility
        content = content.replace('<a href="/" class="nl">Home</a>', '<a href="/" class="nl">Home</a>\n    <a href="javascript:void(0)" onclick="openRate()" class="nl" style="color:#f59e0b;font-weight:700"><i class="fas fa-star"></i> Feedback</a>')
        content = content.replace('<a href="index.html" class="nl">Home</a>', '<a href="/" class="nl">Home</a>\n    <a href="javascript:void(0)" onclick="openRate()" class="nl" style="color:#f59e0b;font-weight:700"><i class="fas fa-star"></i> Feedback</a>')
        
        # Inject our massive UI upgrade before closing body
